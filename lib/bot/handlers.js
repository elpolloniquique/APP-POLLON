...
import { searchKnowledge, bumpKnowledgeUse, saveUnanswered } from './memory.js';
...
export async function handlePayment(ctx) {
  ...
  const pay = hits.find((h) => /pago|transfer|efectivo/i.test(`${h.title} ${h.question} ${h.answer}`));
  if (pay?.answer) {
  ...
  return {
    text: 'En El Pollón el pago es *al recibir*: efectivo o transferencia. No cobramos con tarjeta ni Webpay en la web. Los datos de transferencia te los entrega el local o el repartidor.',
    confidence: 0.9,
