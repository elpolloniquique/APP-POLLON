...
import { phonesMatch, normalizeChilePhone } from './phone.js';
...
export async function findProfileByPhone(admin, phone) {
  const { data } = await admin.from('profiles').select('id, full_name, phone, role, branch_id').limit(200);
  const row = (data || []).find((p) => phonesMatch(p.phone, phone));
  return row || null;
}
