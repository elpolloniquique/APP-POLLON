...
function formatPlato(p, linkWeb, branchId) {
  ...
  const link = storeUrl(linkWeb, { branchId, q: p.name });
...
export async function handleInbound({
  ...
  if (intent === 'queja' || intent === 'humano') {
    ...
    const platoDetalle = top.map((p) => formatPlato(p, settings.link_web, branch.id)).join('\n\n');
    ...
    reply = interpolate(settings.templates.bienvenida, buildTemplateVars({
      branch, settings, name, loyalty,
    }));
  } else {
    reply = interpolate(settings.templates.fallback, buildTemplateVars({
      branch, settings, name, loyalty,
    }));
    intent = 'otro';
  }
  ...
  return sendReply({
    admin, settings, branch, session, phone: normalized, intent, reply, simulate, loyalty,
  });
}
...
async function sendReply({ admin, settings, branch, session, phone, intent, reply, simulate, loyalty }) {
  if (!reply) {
    return { ok: true, intent, reply: null, simulate: !!simulate, loyalty };
  }
  ...
  if (recent >= (settings.rate_limit_per_min || 4)) {
    return { ok: true, skipped: 'rate_limit', intent, reply };
  }
  ...
  if (evolutionConfigured() && settings.evolution_instance) {
    try {
      ...
      await logMessage(admin, {
        ...
        body: reply,
        intent,
        extra: { error: String(err.message || err).slice(0, 300) },
      });
      return { ok: false, error: err.message, intent, reply };
    }
  }
  ...
  return { ok: true, intent, reply, mode: session?.mode || 'bot', loyalty };
}
